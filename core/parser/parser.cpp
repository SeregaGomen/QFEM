...
bool TParser::is_name(string token)
{
    ...
    if (not isalpha(token[0]) and token[0] != '_')
        return false;
...
void TParser::token_or(Tree& code)
{
    ...
    token_and(code);
    while (token_type != TokenType::Finished and tok == Operation::Or)
    {
        get_token();
        token_and(hold);
...
void TParser::token_and(Tree& code)
{
    ...
    token_not(code);
    while (token_type != TokenType::Finished and tok == Operation::And)
    {
        get_token();
        token_not(hold);
...
void TParser::token_not(Tree& code)
{
    ...
    if (token_type == TokenType::Delimiter and op == Operation::Not)
        get_token();
...
void TParser::token_add(Tree& code)
{
    ...
    token_mul(code);
    while (token_type != TokenType::Finished and ((pm = token) == "+" or pm == "-" or pm == ">" or pm == "<" or pm == ">=" or pm == "<=" or pm == "<>" or pm == "=="))
    {
        get_token();
        token_mul(hold);
...
void TParser::token_mul(Tree& code)
{
    ...
    token_pow(code);
    while (token_type != TokenType::Finished and ((pm = token[0]) == '*' or pm == '/'))
    {
        get_token();
        token_pow(hold);
        if (pm == '*')
...
void TParser::token_pow(Tree& code)
{
    ...
    token_un(code);
    if (token_type != TokenType::Finished and token == "**")
    {
        get_token();
        token_cramp(hold);
...
void TParser::token_func(Tree& code)
{
    ...
    {
        get_token();
        if (not token.length() or token[0] != '(')
            error(SYNTAX_ERR);
        get_token();
